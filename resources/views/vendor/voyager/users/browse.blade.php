...
@section('page_title', __('voyager::generic.viewing') . ' ' . \App\Utils\Helpers\Helper::browseUserTitle())

@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            <i class="{{ $dataType->icon }}"></i> {{ \App\Utils\Helpers\Helper::browseUserTitle() }}
        </h1>
        @php
            $canAddNew = true;
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    @foreach($dataType->browseRows as $row)
                                        @if(\App\Utils\Helpers\Helper::showFieldByRole($row))
                                            <th>
                                                @if ($isServerSide)
                                ...
                                @foreach($dataTypeContent as $data)
                                    <tr>
                                        ...
                                        <td class="no-sort no-click bread-actions">
                                            @foreach($actions as $action)
                                                @if (!method_exists($action, 'massAction'))
                                                    @include('voyager::users.actions', ['action' => $action])
                                                @endif
                                            @endforeach
                                        </td>
                                    </tr>
