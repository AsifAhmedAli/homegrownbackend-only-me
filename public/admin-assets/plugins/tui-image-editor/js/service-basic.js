...
var $btnStoneApply = $('#btnStoneApply');
...
$('.apply').on('click', function() {
    var props = imageEditor.getObjectProperties(textObjectId, ['left', 'fontFamily','fontSize', 'top', 'width', 'height', 'fill', 'stroke', 'strokeWidth', 'opacity', 'angle']);
    $('#engravingText').modal('hide');
    let left = props.left.toFixed(2);
    let top = props.top.toFixed(2);
    let angle = props.angle.toFixed(2);
    let fontSize = props.fontSize.toFixed(2);
    $textPositionField.val(left + ',' + top);
    $textPositionField.parent().parent().find('.angle').val(angle)
    $textPositionField.parent().parent().find('.font-size').val(fontSize)
})



imageEditor.on('objectActivated', function(props) {
    textObjectId = props.id
});
...
imageEditor.on('objectRotate', function(props) {
    console.log(props);
});
...
$btnDownload.on('click', () => {
    ...
    if($engravingType === 'stone') {
        //$('.preview-' + $currentImageId).attr('src', dataURL);
       // uploadStoneImage(dataURL, $currentImageId);
        console.log('image Properties', imageProperties)
    } else {
        $('#printImagePreview').empty().append('<img src="' + dataURL + '" class="img-100" />');
        $('.print-image').val(dataURL);
    }

    $('#engravingText').modal('hide');
    $('#btn-mask-filter').addClass('hide');
...
function uploadStoneImage(dataUrl, stoneId) {

    let formData = new FormData;
    formData.append('stone_image', dataUrl);
    formData.append('id', stoneId);
    ...
    error : function(err) {
        console.log(err);
        toastr.error(err.error.message, 'Error', {"preventDuplicates": false, "preventOpenDuplicates": false});
