...
@endif
@php
    $dataTypeContent->{$row->field} = $dataTypeContent->{$row->field};
@endphp

<select class="form-control select2" name="{{ property_exists($options, 'name') ? $options->name : $row->field. '[]' }}" multiple>
    @if(isset($options->relationship))
        ...
        @if( method_exists( $dataType->model_name, camel_case($row->field) ) )
            ...
            @foreach($relationshipOptions as $relationshipOption)
                {{--                @if($row->field == 'product_option_value_ids')--}}
                    <?php
                        $selected_values = [];
                        if(!is_null($dataTypeContent->{$row->field})){
                            $selected_values = json_decode($dataTypeContent->{$row->field});
                        } else if(!is_null(old($row->field))) {
                            $selected_values = old($row->field);
                        }
                    ?>
                 {{--@endif--}}
