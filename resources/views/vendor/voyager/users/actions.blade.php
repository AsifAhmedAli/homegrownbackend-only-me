@if($data)
    ...
    @can ($action->getPolicy(), $data)
        @if($action instanceof \App\Actions\CustomersAction)
            ...
            @endif
        @elseif($action instanceof \App\Actions\ViewPurchasedKitsAction)
            @if($data->kits()->count())
                <a href="{{ $action->getRoute($dataType->name) }}" title="{{ $action->getTitle() }}" {!! $action->convertAttributesToHtml() !!}>
                    <i class="{{ $action->getIcon() }}"></i> <span class="hidden-xs hidden-sm">{{ $action->getTitle() }}</span>
                </a>
            @endif
        @elseif($action instanceof \App\Actions\ViewPurchasedSubscriptionsAction || $action instanceof \App\Actions\AssignCustomerToAction)
            @if($data->subscriptions()->count())
                <a href="{{ $action->getRoute($dataType->name) }}" title="{{ $action->getTitle() }}" {!! $action->convertAttributesToHtml() !!}>
                    <i class="{{ $action->getIcon() }}"></i> <span class="hidden-xs hidden-sm">{{ $action->getTitle() }}</span>
                </a>
            @endif
        @else
            <a @if($action instanceof \TCG\Voyager\Actions\DeleteAction && $data->isAdmin()) style="display: none" @endif href="{{ $action->getRoute($dataType->name) }}?source={{ request()->segment(2, 'users') }}" title="{{ $action->getTitle() }}" {!! $action->convertAttributesToHtml() !!}>
                <i class="{{ $action->getIcon() }}"></i> <span class="hidden-xs hidden-sm">{{ $action->getTitle() }}</span>
            </a>
        @endif
    @endcan
